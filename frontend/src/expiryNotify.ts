...
let expiryDaysBefore = 3;

export function setExpiryDaysBefore(days: number): void {
  if (Number.isFinite(days) && days >= 1) expiryDaysBefore = Math.floor(days);
}

export function getExpiryDaysBefore(): number {
  return expiryDaysBefore;
}
...
/** Напоминание до окончания: осталось ≤N суток, подписка ещё активна. */
export function userExpiryNotifyEligible(u: { expiry_time: number; tg_id: string }): boolean {
  ...
  return left > 0 && left <= expiryDaysBefore * DAY_MS;
}
...
export function formatNotifyExpiryError(raw: string): string {
  ...
  if (raw.includes("too_early")) {
    return `Доступно только в последние ${expiryDaysBefore} сут. до окончания.`;
  }
